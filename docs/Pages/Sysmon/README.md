# Beschreibung und Recherche der Software Sysmon
## Recherche
### Was ist Sysmon? Wofür wird es verwendet
Sysmon ist ein Tool zur Überwachung von Systemereignissen unter Microsoft Windows. Es wurde ursprünglich von Sysinternals entwickelt und ist jetzt Teil von Microsoft. Sysmon wird verwendet, um bestimmte Systemereignisse wie Programmstarts, Netzwerkverbindung, Registy änderungen usw. aufzuzeichnen und zu melden. Sysmon Protokolle enthalten Informationen über laufende Programme, verwendete Netzwerkadressen und die gesetzten Registry Keys.

Sysmon ist ein nützliches und in grossen Umgebungen "unverzichtbares" Tool für Administratoren, um die Systemleistung und -sicherheit zu überwachen und mögliche Probleme oder Bedrohungen zu identifizieren, damit entsprechende Massnahmen ergriffen werden können. Sysmon kann auch verwendet werden, um schädliche Aktivitäten auf dem System zu identifizieren und zu überwachen. Zudem kann Sysmon über die Befehlszeile installiert und konfiguriert werden und protokolliert Ereignisse in der Windows Ereignisanzeige.
### Was ist die aktuellste Version von Sysmon?
Die aktuelle Version von Sysmon ist v14.12 und kann unter folgendem Link heruntergeladen werden: [Download](https://learn.microsoft.com/de-de/sysinternals/downloads/sysmon)
### Was sind Beispiel-Anwendungen/Use-Cases bei welchen Sysmon helfen kann?
Sysmon ist ein Tool von Microsoft, welches es ermöglicht, detaillierte Ereignisprotokolle von Aktivitäten auf einem Windows System zu erstellen. Häufig wird Sysmon in Sicherheitsumgebungen verwendet, um die Überwachung von Systemaktivitäten und die Erkennung von Angriffen oder unerwarteten Ereignissen zu verbessern.

Einige Beispiele für Anwendungen/Use-Cases, bei denen Sysmon helfen kann, sind:
1. Überwachung von Netzwerkaktivitäten: Protokolle von Netzwerkverbindungen und -zugriffen können durch Sysmon erstellt werden, was bei der Überwachung von Netzwerkaktivitäten und der Erkennung von unerwarteten oder verdächtigen Verbindungen hilfreich sein kann. (Z.B. gewisse Clients versuchen sich auf ein Botnetz zu verbinden)
2. Überwachung von Prozessen und Diensten: Sysmon kann Protokolle von gestarteten Prozessen und Diensten generieren, was bei der Überwachung von Systemaktivitäten und der Erkennung von unerwarteten oder verdächtigen Prozessen hilfreich sein kann. (Bspw. Maleware)
3. Überwachung von Registry Änderungen: Registry Änderungen können von Sysmon in einem Protokoll festgehalten werden, was bei der Überwachung von Systemkonfigurationen und der Erkennung von unerwarteten oder verdächtigen Änderungen hilfreich sein kann. (Bspw. User die selber Änderungen an der
4. Überwachung von Dateisystemaktivitäten: Sysmon ist ebenfalls in der Lage Protokolle von Dateizugriffen und -änderungen zu erstellen, was bei der Überwachung von Dateisystemaktivitäten und der Erkennung von unerwarteten oder verdächtigen Änderungen hilfreich sein kann. (Bspw. unerlaubtes kopieren von sensiblen Daten wird protokolliert)
5. Überwachung von Sicherheitsereignissen: Protokolle von Sicherheitsereignissen wie dem Start oder der Beendigung von Sicherheitsdiensten oder dem Einloggen von Benutzern werden durch Sysmon bei entsprechender Konfiguration ebenfalls erstellt, was bei der Überwachung von Sicherheitsaktivitäten und der Erkennung von unerwarteten oder verdächtigen Ereignissen hilfreich sein kann. (Bspw. die Antivierensoftware wird auf einmal getötet)

Dies sind nur einige Beispiele für Sysmon Anwendungen. In den meisten Fällen wird Sysmon verwendet, um Systemereignisse aufzuzeichnen und zu melden, um Systemleistung und -sicherheit zu überwachen und Probleme oder Bedrohungen zu identifizieren und entsprechend zu verfolgen/angemessene Massnahmen einleiten.
## Installation
### Recherchieren und dokumentieren Sie wie man Sysmon manuell auf einem Windows-Server/Client installieren würde
1. Zuerst muss man Sysmon unter folgendem Link Herunterladen: [Download](https://learn.microsoft.com/en-us/sysinternals/downloads/sysmon)
2. Danach kann sich ein Config File erstellen oder ein bestehendes Herunterladen. Bspw. [Sysmon Modular](https://raw.githubusercontent.com/olafhartong/sysmon-modular/master/sysmonconfig.xml). Dies kann man mit folgendem PowerShell Befehl automatisch nach C:\Windows kopieren
```powershell
...
```
3. Nun kann man Sysmon mit folgendem Befehl in PowerShell installieren:
```
#sysmon.exe bei 32 Bit. -accepteula akzeptiert die Lizentvereinbarungen. -i spzifiziert den config.xml Pfad.
...
Sysmon wurde auf den Rechner mit folgendem PowerShell Skript installiert:
```powershell
...
Sysmon wird in PowerShell von [hier](https://live.sysinternals.com/Sysmon64.exe) nach C:\Tools\Sysinternals\Sysmon64.exe heruntergeladen, danch wird es noch nach C:\ProgramData\Sysmon kopiert, und danach wird auch noch das Config File von [Github](https://raw.githubusercontent.com/olafhartong/sysmon-modular/master/sysmonconfig.xml) nach C:\ProgramData\Sysmon\sysmonConfig.xml heruntergelden. Am Schluss wird für die Installation folgender Befehl verwendet:
```powershell
Start-Process -FilePath "$sysmonDir\Sysmon64.exe" -ArgumentList "-accepteula -i $sysmonConfigPath"
```

Das PowerShell Skript wird im Vagrantfile bei jeder Windows VM aufgerufen mit dem Befehl:
```Vagrantfile
cfg.vm.provision "shell", path: "scripts/install-sysinternals.ps1", privileged: false
```

D.h. immer wenn eine VM erstellt/gebaut wird auch Sysmon durch den Befehl im Vagrantfile installiert.
## Konfiguration
### Dokumentieren Sie das Config-File welches auf Ihrem System für Sysmon installiert ist
Das Config File liegt unter C:\ProgramData\Sysmon und heisst sysmonConfig.xml In diesem File sind alle Dinge eingetragen, welche geloggt werden sollen. Bspw. gewisse User aktiviäten oder Services, Applikationen oder das gewisse Filetypes oder Names.
...
Sysmon verwendet als Config File eine XML Struktur. Im XML File sind die Regeln in sogenannten RuleGroups. Diese können entweder mit and oder or versehen werden. Also, ob bei gewissen Dingen alles erfüllt sein muss oder nur gewisse Dinge. Die Rules haben alle eine ID. Diese ID wird dann auch im Eventlog angezeigt. Gewisse Regeln können darunter auch noch einthalten, ob es geloggt werden soll wenn nur gesisse Dinge zutreffen bspw.  <Image condition="is">. Die Struktur ist also Baumartig, ein Ast hat mehrer kleinere Äste die alles genauer definieren -> Die RegelGruppe entahen verschiedene Regln, welche entwerder alle erfüllt sein müssen and oder nur gewisse or oder enthalten direkt Bedigungen. 
