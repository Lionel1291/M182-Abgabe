...
Ich habe bewusst das 10er Netz weggelassen, da es keinerlei Relanz für die funktionalität der Umgebung hat.                     
![Hier sollte ein Screenshot des Netzwerkplan sein](../../Images/Infrastruktur/Netzwerkplan.png "Screenshot des Netzwerkplan")                            
### Beschreibung der beiden Windows-VMs:
#### Login
Das Login findet mit folgenden Zugangsdaten statt:
- Benutzername: vagrant
- Passwort: vagrant
- Logon Domain: WINDOMAIN

#### Installierte Dienste ("Welche Software wurde auf der VM installiert?") + Zugriffe auf die Applikationen ("Wie greife ich auf die verschiedenen Softwaren zu?")
Beide
- OSQuery // PowerShell oder CMD
- Sysmon // Windows EventLog, Handling in PowerShell oder CMD
- Chocolatey //Packet Manager für StartMenu
- Velociraptor // PowerShell oder CMD
- Chocolatey // PowerShell oder CMD
- AutorunsToWinEventLog // PowerShell oder CMD, es wird ein Task im Task Manager erstellt

Windows 10 Rechner
- ShutUp10 // Es gibt eine EXE oder sonst PowerShell oder CMD
...
#### Installierte Software ("Welche Software wurde auf der VM installiert?") + Zugriffe auf die Applikationen ("Wie greife ich auf die verschiedenen Softwaren zu?")
- Fleet // https://192.168.38.105:8412
- OSQuery // Konsole
- Veloceraptor // https://192.168.38.105:9999
- Guacamole // http://192.168.38.105:8080/guacamole
- Suricata // Konsole
- Zeek // Konsole
- Kolide // Über Fleet
- Kibana // http://192.168.38.105:5601
- Filebeat // Konsole
- Auditbeat // Konsole
- Elasticsearch // Über Kibana

## Infrastruktur-Setup
### Sinn und Zweck des Vagrantfile
Mit dem Vagrant File kann ich VMs erstellen und diese Konfiguriren. Durch das Vagrantfile kann ich bspw. automatisiert VMs oder eine ganze Infrastruk erstellen mit den benötigten Konfigurationen. Ich kann das Vagrantfile auch als Template für zukünftige Machienen verwenden. Im File werden nicht nur Skripts hinterlegt, die beim bauen auf dem OS ausgeführt werden, es werden auch die Ressourcen und Netzwerkeinstellungen definiert. 

Bei sehr grossen Umgebungen macht es durchaus sinn so ein File zu benutzen, da man so die Umgebung enfach erweitern kann ohne grossen Aufwand und man hat auch schon ein Template.

In unserem Fall wurde damti das Detection LAB von ELK aufgebaut und Konfiguriert. 

Ein weiterer Vorteil ist, sollte die VM mal kaputt gehen und man hat das File noch, kann man sie ganz einfach mit den selben Einstellungen und Skripts wie zuvor neu bauen und sie ist dann nahezu identisch.

Ich persönlich finde das Vagrantfile eine sehr coole Sache und überlege mir, ob wir das auch bei uns im Betrieb einsetzen können. Denn man kann acuh angeben, welches OS mit welchem Realease genommen werden soll, d.h. man hat immer einen aktuell Build und muss nicht zuerst im lahmen VLSC die neuste ISO herunterladen oder ein neues VMware Template generieren da die VM vom Template alleine schon gefühlt einen Tag Updates machen wird. Dadurch können auch Zeit und Geld gespart werden.
...
Mithilfe der Provison Befehle können PowerShell oder Bash Skripts auf der VM während des "bau Prozesses" automatisiert und mit Parametern ausgeführt werden. Dadurch können einfach Programme installiert, konfigurationen vorgenommen, designfehler ausbesserungen usw. während dem installtionsprozess durchgeführt werden ohne viel Aufwand. Somit ist es auch einfach VMs zu skalieren und diese "gleich" zu bauen.

Beispiel Befehle aus dem Vagrantfile, welche andere Skripts öffnen:
```vagrant
cfg.vm.provision "shell", path: "scripts/fix-second-network.ps1", privileged: true, args: "-ip 192.168.38.103 -dns 8.8.8.8 -gateway 192.168.38.1" 
cfg.vm.provision "shell", path: "scripts/provision.ps1", privileged: false
cfg.vm.provision "shell", path: "scripts/provision.ps1", privileged: false
...
Das bootstrap.sh File nimmt einige Netzwerk konfigurationen vor, installiert einige Hilfsprogramme wie git oder python, installiert einige Server Dienste wie ein mysql Server, konfiguriert osquery, installiert Fleet und konfiguriert dieses, konfiguriert den mysql server und installiert veloceraptor + guacamole + suricata + zeek inkl. Konfiguration. Das Bootstrap File kann man sagen bereitet den Server mit den Basis Tools vor, welche nicht zu ELK gehören und konfiguriert diese Teilweise. 

Das File bootstrap.sh wird vom Vagrantfile aufgerufen und ist ein Bash Skript:
```vagrant
cfg.vm.provision :shell, path: "bootstrap.sh"
```
...
Das ELK.sh-File installiert und konfiguriert Kibana und Elasticsearch auf der Logger VM: Zudem wird auch noch die Umgebungsvariable JAVA_HOME definiert. Bei Kibana werden bspw. Einstellungen getroffen wie was ist die IP Adresse des Hosts usw. Zudem werden noch Sicherheitskonfigration bspw. im bezug auf die Notwendigkeit von Zertifikaten getätigt.                

Das File ELK.sh wird vom Vagrantfile aufgerufen und ist ein Bash Skript:
```vagrant
cfg.vm.provision :shell, path: "ELK.sh"
```
#### ELK.sh Skript
